export default function ShortlistStar({ active, canEdit, onToggle, className = '' }) {
  const base = 'inline-flex items-center justify-center transition-all';
  const color = active
    ? 'text-amber-500 drop-shadow-sm'
    : 'text-pine-200 hover:text-amber-300';

  if (!canEdit) {
    if (!active) return null;
    return (
      <span className={`${base} ${color} ${className}`} title="Shortlisted" aria-label="Shortlisted">
